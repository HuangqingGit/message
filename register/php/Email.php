...
function Insertsuer($db, $Emailid, $pwd)
{
    $ar = rand(3, 5);
    $b = '';
    for ($i = 0; $i < $ar; $i++) {
    ...
    $insert_sql = "INSERT INTO user_form (User_type,User_ID,User_pwd,User_name,User_email) 
        VALUES ('4','{$aray}','{$pwd}','{$b}','{$Emailid}')";
...
$res = Insertsuer($db, $code['Emailid'], md5($code['pwd']));
if ($res['res']) {
    $result = array(
        "code" => true,
        "name" => $res['name'],
